...
함수 : 로직을 모듈화 한 코드
 ==> 실제 사용(호출)하는 곳과 합수가 구현되어있는 부분을 분리 ==> 유지보수의 편이성을 도모
 함수를 사용하지 않을경우
...
제작자 구분 : 
 ...
 . 사용자 정의 함수 : 개발자가 직접 정의한 함수(pl/sq 배울때)
...
SUBSTR(문자열, 시작 인덱스, 종료인덱스) : 문자열의 시작인텍스 부터.... 종료인덱스 까지의 부분 문자열
시작인덱스는 1부터 (*java의 경우는 0부터)

LENGTH(문자열) : 문자열의 길이를 반환

INSTR(문자열, 찾을 문자열, [검색 시작 인덱스]) : 문자열에서 찾을 문자열이 존재하는지, 존재할 경우 찾을 문자열의 인덱스(위치) 반화
...
fn1]
SELECT TO_DATE('2019/12/31', 'YYYY/MM/DD') lastday, TO_DATE('2019/12/31', 'YYYY/MM/DD')-5 lastday_befores, SYSDATE now, SYSDATE - 3 now_before3
FROM dual;


TO_DATE(문자열, 포맷) : 문자열을 포맷에 맞게 해석하여 날짜 타입으로 형변화
...
현재시간(SYSDAY) 시분초 단위까지 표현==> TO_CHAR를 이용하여 형변환;

SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS') now,
        TO_CHAR(SYSDATE, 'D') d,
        TO_CHAR(SYSDATE-3, 'YYYY/MM/DD HH24:MI:SS') now_before3,
        TO_CHAR(SYSDATE-1/24, 'YYYY/MM/DD HH24:MI:SS') now_before3_1hour
FROM dual;
...
SELECT TO_CHAR(TO_DATE(:YYYYMM, 'YYYY/MM'), 'YYYYMM') PARAM, TO_CHAR(LAST_DAY(TO_DATE(:YYYYMM,'YYYY/MM')),'DD')
FROM dual;