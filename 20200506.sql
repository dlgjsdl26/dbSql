...
INSERT INTO emp (empno, ename, hiredate) VALUES(9999, 'heon', SYSDATE);
...
INSERT INTO emp (ename, hiredate) VALUES('LHY', SYSDATE);
...
INSERT = 없던 걸 새로 생성
UPDATE, DELETE = 기존이 있는걸 변경, 삭제
    ==> 쿼리를 작성할 경우 주의
        1. WHERE절이 누락되지 않았는지
        2. UPDATE, DELETE 문을 실행하기 전에 WHERE절을 복사해서 SELECT를 하여 영향이 가는지 행을 눈으로 확인
        
서브쿼리를 이용한 데이터 수정;
INSERT INTO emp(empno, ename, job) VALUES (9999, 'brown', NULL);

9999번 직원의 deptno, job 두개의 컬름을 SMITH 사원의 정보와 동일하게 변경
...
대상 행을 제한하기 위해 WHERE 절을 기술 할 수 있고, WHERE절은 SELECT절에서 사용한 내용을 적용 할 수 있다.
예를들어 서브쿼리를 통한 행의 제한이 가능
...
WHERE 절을 사용 가능한 DML : SELECT, UPDATE, DELETE
    3개의 쿼리는 데이터를 식별화는 WHERE 절이 사용 될 수 있다.
...
ex : ATM~ 출금과 입금이 둘다 정상적으로 이루어져야 문제가 발생되지 않음
            출금은 정상 처리 되었지만 입금이 비정상 처리 되었다면 정상 처리된 출금도 취소를 해줘야 한다.
...
게시글을 하나 등록을 하더라도 게시글 테이블과, 게시글 첨부파일 테이블에 데이터를 신규로 등록을 한다.
