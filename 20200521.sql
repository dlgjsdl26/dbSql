...
SQL ==> SQL실행은 일반 언어에서 실행(java)
        .만약 sql과 관련된 로직이 바뀌면 java파일을 수정할 가능성이 큼
        

PL/SQL : Procedual Language / Structured Query Language
SQL : 집합적, 로직이 없음(미비하게 존재,CASE, DECODE..FOR)

개발을 하다보면 어떤 조건에 따라 조회해야할 테이블 자테가 바뀌거나, 로직을 스킵하는 등의 절차적인 부분이 필요 할 때가 있음

연말정산 : 소득의 25%를 신용카드 + 현금영수증 + 체크카도로 소비를 소비금액이 소득의 25%를 초과하는 금액에 대해서
...
일반적인 프로그래밍 언어에서 사용하는 조건제어(if, case), 반복문(for, while), 변수등을 활용 할 수 있는 PL/SQL 이 등장
...
날짜를 입력받아 ==> 그회사의 영업월기준으로 5일뒤의 날짜를 리턴하는 함수
    회사만의 특이한 로직이 필요한 경우 함수로 만들 수 있다.


PROCEDURE : 이름이 있는 PL/SQL 블록, 리턴값이 없다
            로직에 따라 처리후 데이터를 다른 테이블에 입력하는 등의 비즈니스 로직을 처리 할 때 사용
...
2. PL/SQL RECORD : 사용자가 행을 저장할수 있는 타입, 컬럼을 개발자가 직접 기술
