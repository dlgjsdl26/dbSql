...
테이블의 모든행 중 해당 컬럼에 값은 중복되지 않고 유일해야함
...
CREATE TABLE dept_test(
    ...
    CONSTRAINT u_dept_test UNIQUE (dname, loc) --제약조건명 제약조건TYPE (컬럼,...)
);
...
데이터 입력시 (emp) 참조하는 데이블(dept)에 데이터 존재 유무를 빠르게 알기 위해서
참조하는 테이블(dept)의 컬럼(deptno)에 인덱스가 반드시 생성 되어 있어야만
FOREIGN KEY 제약조건을 추가 할 수 있다.


UNIQUE 제약조건을 생성할 겨우 해당 컬럼의 값 중복 체크를 빠르게 하기 위해서
...
CREATE TABLE emp_test (
    ...
    CONSTRAINT fk_emp_test_dept_test FOREIGN KEY (deptno) /*제약조건 명 제약조건TYPE 컬럼 */REFERENCES dept_test (deptno)--참조하는 테이블(컬럼)
 ); 
...
부모 데이터를 삭제시..
FOREIGN KEY 옵견에 따라 자식 데이터를 처리할 수 있는 옵션
1. default ==> 참조하고 있느 부모가 삭제 될 수 없다.
...
2. 테이블 명세서가 정확하지 않으면 신규 개발자는 해당 내용을 모를 수가 있음
    (java코드 + 테이블 내역 확인 필요)
    jaca코드에는 dept테이블만 삭제하는 코드가 있는데
    신기하게도 emp테이블의 데이터가 삭제되거나 null로 설정되는 경우를 볼 수 있음.
    
CHECK 제약조건
컬럼의 값을 제한하는 제약 조건

emp 테이블에서 급여정보(sal)를 관리 할 때 sal 컬럼의 값은 0보다 작은 값이 들어가는 것은 로직적으로 이상함.
...
sal컬럼에 설정된 check 제약조건
INSERT INTO emp_test (9998, 'sally', -1000, 99); 
...
CREATE TABLE emp_test (
    ...
    CONSTRAINT check_sal CHECK (sal > 0)--제약조건이름 제약조건 타입 (컬럼)
 );
...
이미 생성된 테이블을 수정 할 수도 있음
    ...
    2. 기존 컬럼 삭제, 컬럼 이름 변경, 컬럼 사이즈 변경, (타입도 제한적으로 변경 가능)
        ...
        (알아서 이름을 변경 해준다)
        emp_tset.deptno ==> dept_test.deptno 참조
